...
contract('Timelock', ([alice, bob, carol, dev, minter]) => {
    beforeEach(async () => {
        this.prex = await PrexToken.new({ from: alice });
        this.timelock = await Timelock.new(bob, '28800', { from: alice }); //8hours
    });

    it('should not allow non-owner to do operation', async () => {
        await this.prex.transferOwnership(this.timelock.address, { from: alice });
        await expectRevert(
            this.prex.transferOwnership(carol, { from: alice }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.prex.transferOwnership(carol, { from: bob }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.timelock.queueTransaction(
                this.prex.address, '0', 'transferOwnership(address)',
    ...
    it('should do the timelock thing', async () => {
        ...
        assert.equal((await this.prex.owner()).valueOf(), carol);
    });
    ...
    it('should also work with MasterChef', async () => {
        ...
        this.chef = await MasterChef.new(this.prex.address, this.syrup.address, dev, '1000', '0', { from: alice });
        await this.prex.transferOwnership(this.chef.address, { from: alice });
